// ─────────────────────────────────────────────────────────────────────────────
// TEJAS LIBRARY MANAGEMENT SYSTEM — v3.0  (Bug-Fixed & Modular)
//
// BUG FIXES IN THIS VERSION:
//
// [FIX-1] REVENUE DUPLICATION (mark→unmark→mark doubles revenue)
//   Root cause: applyMarkPaid was adding a renewal entry on every "Mark Paid" click.
//   Fix: Mark Paid / Unmark ONLY toggle the `paid` boolean. They never touch `renewals`.
//   Revenue = sum(renewals[].amount) is now immutable from mark/unmark actions.
//
// [FIX-2] MANUAL ENTRY + MARK PAID = duplicate revenue
//   Root cause: Manual entry creates renewals, then "Mark Paid" added another one.
//   Fix: Same as FIX-1. Mark Paid never creates renewals; renewals only come from
//   RenewModal, the new-member form (first-join), or the Manual Entry tab.
//
// [FIX-3] UNMARK not removing revenue
//   Root cause: Unmark set paid=false but left the renewal entry that Mark Paid had added.
//   Fix: FIX-1 eliminates the renewal from Mark Paid entirely, so Unmark is clean.
//
// [FIX-4] New member with paid=true correctly creates ONE first-join renewal
//   (this is intentional — it IS a real payment event).
//   But clicking "Mark Paid" on a member who already has renewals no longer adds more.
//
// [FIX-5] All state properly modularised into focused hooks and utility functions.
//
// ─────────────────────────────────────────────────────────────────────────────

import {
  useState, useRef, useCallback, useEffect, useMemo, createContext, useContext,
} from "react";
...
const daysLeft  = (e) => Math.ceil((new Date(e)-new Date(todayStr()))/86400000);
...
const normalizeSeat = (v) => { if(v===""||v==null) return null; const n=parseInt(v,10); return isNaN(n)?null:n; };

const genId = (existingIds=[]) => {
  const existing=new Set(existingIds); let id;
  do { const ts=Date.now().toString(36).slice(-4).toUpperCase(); const r=Math.random().toString(36).slice(2,5).toUpperCase(); id="LIB"+ts+r; } while(existing.has(id));
  return id;
};
...
const DEFAULT_SETTINGS = {
  libraryName:"Tejas Library", totalSeats:35, defaultFee:500, address:"", timing:"",
};
...
const ops = {
  ...
  editMember: (members, id, fields, seatNum) =>
    members.map(m => {
      ...
      // Evict another active/expiring member from the same seat
      const st = getMemberStatus(m);
      if (seatNum!=null && toSeatInt(m.seatNo)===seatNum && m.id!==id &&
  ...
  toggleActive: (members, id) =>
    members.map(m => m.id!==id ? m : { ...m, manualInactive:!m.manualInactive }),

  // Delete
  deleteMember: (members, id) => members.filter(m => m.id!==id),
};
...
const makeC = (dark) => {
  ...
  _C_CACHE[key] = {
    ...
    teal:         "#0D9488", tealDark:"#0B7C73",
    tealLight:    dark?"#0D948822":"#F0FDFA",
    tealMid:      dark?"#0D948833":"#CCFBF1",
    tealGrad:     "linear-gradient(135deg,#0D9488 0%,#0EA5A0 100%)",
    amber:        "#D97706", amberLight:dark?"#D9770622":"#FEF3C7",
    red:          "#DC2626", redLight:  dark?"#DC262622":"#FEE2E2",
    green:        "#16A34A", greenLight:dark?"#16A34A22":"#DCFCE7",
    purple:       "#9333EA", purpleLight:dark?"#9333EA22":"#F3E8FF",
    blue:         "#2563EB", blueLight:  dark?"#2563EB22":"#EFF6FF",
    orange:       "#EA580C", orangeLight:dark?"#EA580C22":"#FFF7ED",
    indigo:       "#4F46E5", indigoLight:dark?"#4F46E522":"#EEF2FF",
...
const sCard   = (C,x={}) => ({background:C.surface,border:`1px solid ${C.border}`,borderRadius:RADIUS.lg,boxShadow:SH,...x});
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 5: PRIMITIVE COMPONENTS
// ─────────────────────────────────────────────────────────────────────────────
...
const Alert = ({children,color,bg,iconName,style={}}) => {
  const {dark}=useDark(); const C=makeC(dark);
  return (
    <div style={{background:dark?color+"22":bg,border:`1px solid ${color}25`,borderLeft:`3px solid ${color}`,borderRadius:RADIUS.md,padding:"11px 14px",display:"flex",gap:10,alignItems:"flex-start",boxShadow:SH_XS,...style}}>
...
const Btn = ({children,onClick,variant="primary",size="md",disabled=false,full=false,iconName}) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{background:v.bg,color:v.color,border:v.border,...sz[size],borderRadius:RADIUS.md,fontWeight:700,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.5:1,width:full?"100%":"auto",fontFamily:FONT,transition:TR,boxShadow:disabled?"none":hov?SH_MD:SH_SM,transform:press?"scale(0.97)":"scale(1)",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:sz[size].gap}}>
...
const Field = ({label,value,onChange,type="text",placeholder,hint,required,options,disabled=false,readOnly=false,onKeyDown}) => {
  ...
  const base={width:"100%",padding:"10px 12px",borderRadius:RADIUS.md,border:`1.5px solid ${foc?C.teal:C.border}`,fontSize:14,color:disabled||readOnly?C.faint:C.text,background:disabled||readOnly?C.surfaceAlt:C.surface,outline:"none",boxSizing:"border-box",fontFamily:FONT,boxShadow:foc?FOCUS:"none",transition:TR,cursor:readOnly?"not-allowed":"text"};
  return (
    <div style={{marginBottom:16}}>
      {label?<label style={{display:"block",fontSize:11,fontWeight:700,color:C.sub,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.07em"}}>{label}{required?<span style={{color:"#DC2626"}}> *</span>:null}</label>:null}
      {options
        ...
        :<input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} disabled={disabled} readOnly={readOnly} onFocus={()=>setFoc(true)} onBlur={()=>setFoc(false)} onKeyDown={onKeyDown} style={base}/>
      }
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.68)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:C.surface,borderRadius:RADIUS.xl,padding:28,maxWidth:300,width:"100%",boxShadow:SH_LG,textAlign:"center"}}>
        ...
        <p style={{fontSize:13,color:C.sub,marginBottom:20}}>Bina save kiye band karna chahte hain?</p>
        <div style={{display:"flex",gap:10}}>
...
const DarkToggle = () => {
  ...
  return (
    <button onClick={toggle} style={{background:"none",border:"none",cursor:"pointer",padding:"6px 8px",borderRadius:RADIUS.md,display:"flex",alignItems:"center",gap:6,color:dark?"#F59E0B":"#475569",transition:TR}}>
...
const Receipt = ({member,renewal,libName,onClose}) => (
  <Modal title="Payment Receipt" onClose={onClose}>
    ...
    <div style={{marginBottom:16}}>
      {[["Member",member.name],["ID",member.id],["Phone",member.phone],["Plan",renewal.planName],["Amount",`₹${renewal.amount}`],["From",fmtDate(renewal.from)],["To",fmtDate(renewal.to)],["Paid On",`${fmtDate(renewal.paidOn)} ${renewal.paidTime||""}`]].map(([k,v])=>(
...
const MemberIdCard = ({member,libName,libAddress,onClose}) => {
  ...
  const printCard = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>ID Card — ${member.name}</title>
<style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Inter',sans-serif;background:#f0f4f8;display:flex;align-items:center;justify-content:center;min-height:100vh;}.card{width:340px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.18);border:1px solid #e2e8f0;}.header{background:linear-gradient(135deg,#0D9488,#0EA5A0);padding:18px 20px 16px;color:#fff;}.lib-name{font-size:17px;font-weight:900;}.lib-sub{font-size:10px;opacity:.82;margin-top:2px;}.body{padding:18px 20px;}.avatar{width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,#0D9488,#14B8A6);color:#fff;font-weight:900;font-size:22px;display:flex;align-items:center;justify-content:center;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,.18);margin-bottom:12px;}.name{font-size:18px;font-weight:900;color:#0f172a;}.mid{font-size:12px;color:#475569;margin-top:2px;font-family:monospace;font-weight:600;}.row{display:flex;justify-content:space-between;margin-top:10px;font-size:11px;}.label{color:#94a3b8;font-weight:600;}.val{color:#0f172a;font-weight:700;}.badge{display:inline-block;padding:3px 10px;border-radius:999px;font-size:10px;font-weight:700;background:${sm.bg};color:${sm.color};margin-top:8px;}.footer{border-top:1px solid #e2e8f0;padding:10px 20px;font-size:9px;color:#94a3b8;text-align:center;}@media print{body{background:#fff;}.card{box-shadow:none;}}</style>
</head><body><div class="card"><div class="header"><div class="lib-name">${libName}</div><div class="lib-sub">${libAddress} • Library Membership Card</div></div>
<div class="body"><div class="avatar">${member.name[0]}</div><div class="name">${member.name}</div><div class="mid">${member.id}</div><div class="badge">${sm.label}</div>
...
<div class="row"><span class="label">Valid Till</span><span class="val">${fmtDate(member.expiry)}</span></div></div>
<div class="footer">Tejas Library • Official Member Card • Not Transferable</div></div>
<script>window.onload=()=>{window.print();setTimeout(()=>window.close(),800);}<\/script></body></html>`);
    w.document.close();
  };
  return (
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 7: RENEW MODAL
// ─────────────────────────────────────────────────────────────────────────────

const RenewModal = ({member,plans,onRenew,onClose}) => {
  const {dark}=useDark(); const C=makeC(dark);
  const [selPlanId,setSelPlanId]=useState(member.planId||plans[0]?.id);
  const [amount,setAmount]=useState(()=>{const p=plans.find(x=>x.id===(member.planId||plans[0]?.id)); return p?String(p.price):"";});
...
const MemberTimeline = ({member,plans,onRenew,onReceipt,onIdCard}) => {
  ...
  const periodDays=lastRenewal?Math.max(1,Math.ceil((new Date(lastRenewal.to)-new Date(lastRenewal.from))/86400000)):(plan?.days||30);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14}}>
        ...
        <StatBox icon="fee"   label="Total Revenue" value={`₹${revTotal.toLocaleString()}`} color="#9333EA"/>
      </div>
...
const SeatsScreen = ({members,settings}) => {
  ...
  members.forEach(m=>{
    const st=getMemberStatus(m);
    if(m.seatNo!=null&&(st==="active"||st==="expiring")) occupied[toSeatInt(m.seatNo)]=m;
  });
  return (
    <div>
        ...
        {Array.from({length:total},(_,i)=>{
          ...
          return (
            <div key={num} style={{background:bg,border:`2px solid ${color}`,borderRadius:RADIUS.md,padding:"10px 6px",textAlign:"center",boxShadow:SH_XS}}>
              ...
              {member?<div style={{fontSize:9,color,fontWeight:700,marginTop:2,lineHeight:1.2,wordBreak:"break-word"}}>{member.name.split(" ")[0]}</div>:<div style={{fontSize:9,color:"#16A34A",marginTop:2}}>Free</div>}
            </div>
          );
        })}
      </div>
...
const MembersScreen = ({members,setMembers,plans,setPlans,settings,addAudit,currentUser}) => {
  ...
  // Seats occupied by active/expiring members only
  const occupiedSeats=useMemo(()=>new Set(
    members.filter(m=>{const st=getMemberStatus(m);return st==="active"||st==="expiring";})
           .filter(m=>m.seatNo!=null).map(m=>toSeatInt(m.seatNo))
  ),[members]);
  ...
  const save=()=>{
    ...
    if(!form.phone.trim()||!/^\d{10}$/.test(form.phone.trim())){alert("Valid 10-digit phone number required.");return;}
    ...
    if(editing){
      ...
      setMembers(prev=>[
        ...prev.map(m=>(seatNum!=null&&toSeatInt(m.seatNo)===seatNum&&(getMemberStatus(m)==="active"||getMemberStatus(m)==="expiring"))?{...m,seatNo:null}:m),
        newM,
      ]);
  ...
  const doDeactivate=id=>{
    ...
    setMembers(prev=>ops.toggleActive(prev,id));
  ...
  const doDelete=id=>{
    const m=members.find(x=>x.id===id);
    setMembers(prev=>ops.deleteMember(prev,id));
    ...
    <div>
            ...
            <Card key={m.id} style={{padding:14}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:C.faint,marginTop:2}}>Since {fmtDate(m.firstJoined)} · {m.renewals?.length||0} sessions · ₹{rev.toLocaleString()} total</div>
                </div>
      ...
      {modal&&(
        <Modal title={editing?"Edit Member":"New Member"} onClose={tryClose}>
          ...
          {!editing&&<Alert color="#2563EB" bg="#EFF6FF" iconName="info" style={{marginBottom:14}}><b>First Join Date — Auto Locked:</b> {fmtDate(todayStr())}</Alert>}
          <Divider label="Seat"/>
      ...
      {viewing&&(
        <Modal title="Member Profile" onClose={()=>setViewing(null)} wide>
          {(()=>{const fm=members.find(x=>x.id===viewing.id)||viewing; return (
...
const AdminPanel = ({settings,setSettings,plans,setPlans,members,setMembers,staff,setStaff,auditLog,addAudit,currentUser}) => {
  ...
  useEffect(()=>{setS({...settings});setOrigS({...settings});},[settings]);
  ...
  const doDeletePlan=id=>{const p=plans.find(x=>x.id===id);setPlans(prev=>prev.filter(x=>x.id!==id));addAudit(currentUser,`Plan deleted: ${p?.name}`);setDeletePlanGuard(null);};
  ...
  const saveStaff=()=>{
    ...
    if(editStaff){
      ...
      if(existing?.role==="superadmin"&&sfForm.role!=="superadmin"){alert("Superadmin ka role change nahi ho sakta.");return;}
  ...
  const toggleStaffActive=id=>{
    ...
    if(sf?.role==="superadmin"){alert("Superadmin account ko deactivate nahi kar sakte.");return;}
  ...
  const doDeleteStaff=id=>{
    const sf=staff.find(x=>x.id===id);
    if(id===currentUser.id){setDeleteStaffGuard(null);alert("Aap apna khud ka account delete nahi kar sakte.");return;}
    if(sf?.role==="superadmin"){setDeleteStaffGuard(null);alert("Superadmin account delete nahi ho sakta.");return;}
  ...
  const exportData=()=>{
    ...
    const a=document.createElement("a");a.href=url;a.download=`tejas-library-backup-${todayStr()}.json`;a.click();
      ...
      {tab==="manual"&&(
        <div>
          ...
          <Card style={{padding:20}}>
            ...
            {manualSaved&&<Alert color="#16A34A" bg="#DCFCE7" iconName="check" style={{marginTop:14,marginBottom:0}}>Member successfully add ho gaya!</Alert>}
...
const LoginScreen = ({staff,onLogin}) => {
  ...
  const handleLogin=()=>{
    const user=staff.find(s=>s.email===email.trim()&&String(s.pin)===String(pin).trim()&&(s.active===true||s.active==="true"||s.active===1));
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:FONT}}>
      <div style={{width:"100%",maxWidth:360}}>
        ...
        <Card style={{padding:24}}>
          ...
          <Btn onClick={handleLogin} iconName="shield" full variant="primary">Login</Btn>
        </Card>
        <div style={{textAlign:"center",marginTop:16}}><DarkToggle/></div>
      </div>
    </div>
  );
};
...
function useSupabaseSync({members,setMembers,plans,setPlans,settings,setSettings,staff,setStaff,auditLog,setAuditLog,setLoading,currentUser}) {
  ...
  const membersTimer=useRef(null); const settingsTimer=useRef(null);
  const plansTimer=useRef(null);   const staffTimer=useRef(null);
  const auditTimer=useRef(null);

  const killAllTimers=useCallback(()=>{
    clearTimeout(membersTimer.current);clearTimeout(settingsTimer.current);
    clearTimeout(plansTimer.current);clearTimeout(staffTimer.current);clearTimeout(auditTimer.current);
    membersTimer.current=settingsTimer.current=plansTimer.current=staffTimer.current=auditTimer.current=null;
  },[]);

  useEffect(()=>{
    if(!supabase||!currentUser){killAllTimers();if(!supabase)return;setLoading(false);return;}
    killAllTimers();setSynced(false);setSyncError(null);
    let cancelled=false;
    const load=async()=>{
      ...
      try{
        ...
        if(!mRes.error&&mRes.data&&mRes.data.length>0)
          setMembers(mRes.data.map(m=>({id:m.id,name:m.name||"",phone:m.phone||"",address:m.address||"",planId:m.planid||"",seatNo:normalizeSeat(m.seatno),firstJoined:m.firstjoined||m.expiry||todayStr(),expiry:m.expiry||todayStr(),paid:!!m.paid,manualInactive:!!m.manualinactive,renewals:Array.isArray(m.renewals)?m.renewals:[]})));
        else if(!mRes.error) setMembers([]);
        if(!pRes.error&&pRes.data&&pRes.data.length>0) setPlans(pRes.data);
        else if(!pRes.error) setPlans([]);
        if(!sRes.error&&sRes.data) setSettings({libraryName:sRes.data.libraryname||DEFAULT_SETTINGS.libraryName,totalSeats:Number(sRes.data.totalseats)||DEFAULT_SETTINGS.totalSeats,defaultFee:Number(sRes.data.defaultfee)||DEFAULT_SETTINGS.defaultFee,address:sRes.data.address||DEFAULT_SETTINGS.address,timing:sRes.data.timing||DEFAULT_SETTINGS.timing});
        if(!stRes.error&&stRes.data&&stRes.data.length>0) setStaff(stRes.data.map(s=>({...s,active:s.active===true||s.active==="true"||s.active===1})));
        else if(!stRes.error) setStaff([]);
        if(!aRes.error&&aRes.data&&aRes.data.length>0) setAuditLog(aRes.data.map(r=>({by:r.by||"?",action:r.action||"",at:r.at||""})));
        if(!cancelled) setSynced(true);
      }catch(e){if(!cancelled)setSyncError("Supabase load failed: "+e.message);}
      finally{setLoading(false);if(!cancelled)setSyncing(false);}
    };
    load();
    return()=>{cancelled=true;killAllTimers();};
  },[currentUser,killAllTimers]);

  const saveMembers=useCallback(async(data)=>{
    if(!supabase||data.length===0) return;
    const rows=data.map(m=>({id:m.id,name:m.name,phone:m.phone,address:m.address||"",planid:m.planId,seatno:normalizeSeat(m.seatNo),firstjoined:m.firstJoined,expiry:m.expiry,paid:!!m.paid,manualinactive:!!m.manualInactive,renewals:m.renewals||[],updated_at:new Date().toISOString()}));
    const {error}=await supabase.from("members").upsert(rows,{onConflict:"id"});
    if(error) console.error("Members upsert:",error.message);
    const {data:dbRows,error:fetchErr}=await supabase.from("members").select("id");
    if(!fetchErr&&dbRows){const localIds=new Set(data.map(m=>m.id));const toDelete=dbRows.map(r=>r.id).filter(id=>!localIds.has(id));if(toDelete.length>0){const{error:delErr}=await supabase.from("members").delete().in("id",toDelete);if(delErr)console.error("Members delete:",delErr.message);}}
  },[]);

  const saveSettings=useCallback(async(data)=>{
    if(!supabase) return;
    const{error}=await supabase.from("settings").upsert({id:1,libraryname:data.libraryName,totalseats:data.totalSeats,defaultfee:data.defaultFee,address:data.address,timing:data.timing},{onConflict:"id"});
  ...
  const savePlans=useCallback(async(data)=>{
    ...
    if(error){console.error("Plans upsert:",error.message);return;}
    const{data:dbPlans,error:fetchErr}=await supabase.from("plans").select("id");
    if(!fetchErr&&dbPlans){const localIds=new Set(data.map(p=>p.id));const toDelete=dbPlans.map(p=>p.id).filter(id=>!localIds.has(id));if(toDelete.length>0){const{error:delErr}=await supabase.from("plans").delete().in("id",toDelete);if(delErr)console.error("Plans delete:",delErr.message);}}
  },[]);

  const saveStaff=useCallback(async(data)=>{
    if(!supabase) return;
    if(data.length>0){const{error}=await supabase.from("staff").upsert(data,{onConflict:"id"});if(error)console.error("Staff save:",error.message);}
    const{data:dbStaff,error:fetchErr}=await supabase.from("staff").select("id");
    if(!fetchErr&&dbStaff){const localIds=new Set(data.map(s=>s.id));const toDelete=dbStaff.map(s=>s.id).filter(id=>!localIds.has(id));if(toDelete.length>0){const{error:delErr}=await supabase.from("staff").delete().in("id",toDelete);if(delErr)console.error("Staff delete:",delErr.message);}}
  },[]);
  ...
  const saveAuditLog=useCallback(async(data)=>{
    ...
    const newRows=data.map(r=>({by:r.by,action:r.action,at:r.at})).filter(r=>!existingKeys.has(`${r.at}||${r.by}||${r.action}`));
    if(!newRows.length) return;
    const{error}=await supabase.from("audit_log").insert(newRows);
    if(error) console.error("Audit log save:",error.message);
  },[]);

  useEffect(()=>{if(!supabase||!synced||!currentUser)return;clearTimeout(membersTimer.current);membersTimer.current=setTimeout(()=>saveMembers(members),800);return()=>clearTimeout(membersTimer.current);},[members,synced,currentUser,saveMembers]);
  useEffect(()=>{if(!supabase||!synced||!currentUser)return;clearTimeout(settingsTimer.current);settingsTimer.current=setTimeout(()=>saveSettings(settings),800);return()=>clearTimeout(settingsTimer.current);},[settings,synced,currentUser,saveSettings]);
  useEffect(()=>{if(!supabase||!synced||!currentUser)return;savePlans(plans);},[plans,synced,currentUser,savePlans]);
  useEffect(()=>{if(!supabase||!synced||!currentUser)return;clearTimeout(staffTimer.current);staffTimer.current=setTimeout(()=>saveStaff(staff),800);return()=>clearTimeout(staffTimer.current);},[staff,synced,currentUser,saveStaff]);
  useEffect(()=>{if(!supabase||!synced||!currentUser||!auditLog.length)return;saveAuditLog(auditLog);},[auditLog,synced,currentUser,saveAuditLog]);
...
export default function App() {
  const [dark,setDark]=useState(()=>{try{return localStorage.getItem("lib_dark")==="1";}catch{return false;}});
  const toggleDark=()=>setDark(d=>{try{localStorage.setItem("lib_dark",d?"0":"1");}catch{} _C_CACHE[d?"light":"dark"]=null; return!d;});
  const C=makeC(dark);

  const [currentUser,setCurrentUser]=useState(()=>{try{const s=localStorage.getItem("lib_user");return s?JSON.parse(s):null;}catch{return null;}});
  useEffect(()=>{try{if(currentUser)localStorage.setItem("lib_user",JSON.stringify(currentUser));else localStorage.removeItem("lib_user");}catch{}},[currentUser]);
  ...
  const [loading,setLoading]=useState(()=>{try{return!!localStorage.getItem("lib_user");}catch{return false;}});

  // Pre-load staff for login screen
  useEffect(()=>{
    if(!supabase||currentUser) return;
    supabase.from("staff").select("*").then(({data,error})=>{
      if(error) console.error("Staff pre-load:",error.message);
      else if(data&&data.length>0) setStaff(data.map(s=>({...s,active:s.active===true||s.active==="true"||s.active===1})));
      setLoading(false);
    });
  },[]);// eslint-disable-line

  const {syncing,synced,syncError}=useSupabaseSync({members,setMembers,plans,setPlans,settings,setSettings,staff,setStaff,auditLog,setAuditLog,setLoading,currentUser});

  const addAudit=useCallback((user,action)=>{setAuditLog(prev=>[...prev,{by:user?.name||"System",action,at:fmtDT()}]);},[]);
    ...
    <DarkCtx.Provider value={contextValue}>
      <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT,padding:24}}>
        <div style={{textAlign:"center",maxWidth:340}}>
          <div style={{width:64,height:64,background:"#FEE2E2",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}><Icon name="warn" size={28} color="#DC2626"/></div>
          <div style={{fontSize:18,fontWeight:900,color:C.text,marginBottom:8}}>Supabase Not Configured</div>
          <div style={{fontSize:13,color:C.sub,lineHeight:1.6}}><code>VITE_SUPABASE_URL</code> aur <code>VITE_SUPABASE_ANON_KEY</code> environment variables set nahi hain.<br/><br/><code>.env</code> file mein dono values add karo aur app restart karo.</div>
        </div>
      </div>
    </DarkCtx.Provider>
  );
  ...
  if(loading) return (
    <DarkCtx.Provider value={contextValue}>
      <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT}}>
        <div style={{textAlign:"center"}}><div style={{fontSize:28,marginBottom:8}}>⟳</div><div style={{fontSize:14,color:"#0D9488",fontWeight:700}}>Loading...</div></div>
      </div>
    </DarkCtx.Provider>
  );
      ...
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:FONT,paddingBottom:72,transition:"background 0.2s"}}>
        ...
        <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50,boxShadow:SH_SM}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            {supabase&&<span style={{fontSize:10,color:syncing?"#D97706":synced?"#16A34A":"#94A3B8",fontWeight:600}}>{syncing?"⟳ Syncing":synced?"● Synced":"○"}</span>}
            <DarkToggle/>
            <button onClick={()=>{setCurrentUser(null);setScreen("dashboard");setLoading(false);setAuditLog([]);}}
        ...
        {syncError&&<div style={{background:"#FEE2E2",borderBottom:"1px solid #FCA5A5",padding:"8px 16px",fontSize:12,color:"#DC2626",fontWeight:600}}>⚠ {syncError}</div>}
